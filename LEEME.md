...
**Impresora (PC de caja).** En el PC que tiene la impresora, entra a **Caja → Impresora → Descargar acceso directo para el PC de caja**. Se baja `LosChamos-Caja.bat`: con doble clic abre la app en Chrome, imprime sin preguntar y deja ese PC como estación de impresión (ingresa una vez; la sesión dura 14 días). Pon la térmica como impresora predeterminada con corte automático y prueba con **Imprimir prueba**. También se puede activar a mano con "Imprimir en este equipo". Ese PC imprime solo, cada pocos segundos, las comandas, pre-cuentas y recibos que pidan desde cualquier celular (deja la app abierta ahí, no minimizada). En **Ver impresiones y reimprimir** se saca otra vez un papel que salió mal. Si en Pedidos aparece "Hay impresiones esperando en caja", el PC de caja no tiene la app abierta o no tiene activada la opción.

**Extras y domicilios.** Si una categoría tiene extras (Ajustes → Menú, ej. "Queso extra: 3000"), el producto se agrega de un toque y debajo aparecen botones "+ Queso extra" para marcarlos solo si el cliente los pide. Un domicilio pide nombre, dirección, valor del envío y cómo va a pagar el cliente; al cobrarlo, el envío se resta solo del efectivo (es lo que se le da al mensajero), así la caja cuadra aunque el cliente pague por transferencia.
